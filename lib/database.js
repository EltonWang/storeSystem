...
function searchSlot(searchKey, callback){
    pool.getConnection(function(err, connection){
        console.log('Get Connection.');
        connection.query('SELECT * FROM `store`.`slot` WHERE batch LIKE \'%' + searchKey + '%\'', function(err, result, fields){
            //console.log(result);
            console.log("Search Slot data with: " + searchKey);
            connection.release();
            callback(result);
        });
    });
